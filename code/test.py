...
def get_conn_prob(dist_mm, circadian):
    ...
    p_raw = nest.math.minimum(1.0, nest.math.maximum(0.0, p_raw)) 
    # zero beyond 250 µm
...
def run_simulation(sim_parameters, condition, rngseed = '', verbose=False, circadian='ZT0'):
    ...
    p_out_pe = pv2e_tail_p(N_pe, n_pv)
...
if __name__=="__main__":
    ...
    try:
        ...
        # Optional 5th CLI arg: sigma override (in mm)
        sigma_override = float(sys.argv[5]) if len(sys.argv) > 5 else None
        if sigma_override is not None:
            sim_parameters['stim_sigma_mm'] = sigma_override
