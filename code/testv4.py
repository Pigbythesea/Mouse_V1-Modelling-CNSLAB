# note: this is adapted from testv2.py
# increased size and bar width logic
...
# --- put right after imports in testv2.py ---
USE_RECT_STIM  = True    # True => rectangle; False => keep Gaussian (legacy)
RECT_CENTER_X  = 1.00    # center of the vertical strip on the 1-mm torus
...
def pv2e_tail_p(N_pe_target, n_pv, p_local=PV2E_LOCAL_P, r_local=PV2E_LOCAL_R_MM, size_mm=1.0):
    # fraction of presynaptic PV within r_local on the torus
    frac_local = (np.pi * (r_local**2)) / (size_mm*size_mm)          # area fraction
...
def run_simulation(sim_parameters, condition, rngseed = '', verbose=False, circadian='ZT0', bar_width_deg=None):
    ...
    ### Network Parameters ###
    n_cells       = 40000               # Total number of cells in the recurrent network
    n_thalamus    = 4000                # Total number of cells in the input layer
    n_exc         = 32000
    n_inh         = n_cells - n_exc
    n_sst         = int(n_inh/2)
    n_pv         = int(n_inh/2)
    size          = 2.0                 # Size of the network
    ...
    exc_pos = nest.spatial.free(pos=nest.random.uniform(min=0., max=1.),
                               extent=[size,size], edge_wrap=False)
    ...
    ratio_in = (np.pi * (se_lat**2)) / (size*size)
    ...
    p_out_pe = pv2e_tail_p(N_pe, n_pv, size_mm=size)
    d = nest.spatial.distance
    p_local = PV2E_LOCAL_P
    p_tail  = p_out_pe
    p2e_mask = nest.logic.conditional(d <= PV2E_LOCAL_R_MM, p_local, nest.logic.conditional(d <= PV2E_MAX_R_MM, p_tail, 0.0))
    p2e_conn = {'rule': 'pairwise_bernoulli', 'p': p2e_mask}
    p2e_syn = {'synapse_model':'static_synapse','receptor_type': 2, 'weight': g_syn_pe, 'delay': 0.1 + d/velocity}
    ...
    try:
        if USE_RECT_STIM:
            theta_deg = float(bar_width_deg) if bar_width_deg is not None else float(sim_parameters.get('stim_bar_width_deg', 0.0))
            w0 = (10.0/180.0)*size                       # 10° RF over the 2-mm width
            weff = min(w0 + theta_deg, size)             # add θ° in mm; clamp to Lx
            sim_name += f"_wd{theta_deg:.3f}deg_we{weff:.3f}mm"
        else:
        ...
        for ni in range(n_thalamus):
            ...
            for repeat in range(nb_repeats):
                ...
                # runtime width (default 0.20mm if not overridden)
                    theta_deg = float(sim_parameters.get('stim_bar_width_deg', 0.0))
                    w0 = (10.0/180.0)*size
                    rect_w = min(w0 + theta_deg, size)                      # your rule; no x-wrap
                    inside = inside_vertical_strip_no_wrap(x, RECT_CENTER_X, rect_w, L=size)
    ...
    ### Median FR ###
    exc_indices = np.arange(1,n_exc+1)
    pv_indices = np.arange(n_exc+1,n_exc+n_pv+1)
    sst_indices = np.arange(n_exc+n_pv+1,n_exc+n_pv+n_sst+1)
...
if __name__=="__main__":
    ...
    try:
        ...
        # Optional 5th CLI arg: BAR WIDTH IN DEGREES (your mapping adds this to the 10° baseline)
        bar_width_deg = float(sys.argv[5]) if len(sys.argv) > 5 else None
        if bar_width_deg is not None:
            sim_parameters['stim_bar_width_deg'] = bar_width_deg  # runtime-only override
    ...
    run_simulation(sim_parameters, condition, rngseed = rngseed, circadian = circadian, bar_width_deg=bar_width_deg)
