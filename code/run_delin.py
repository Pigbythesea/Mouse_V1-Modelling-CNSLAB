...
def run_simulation(sim_parameters, condition, rngseed = ''):
    ...
    del_range = sim_parameters['del_range']
    ...
    s_sst         = 1.                  # Spread of somatostatin connections
    ...
    n2e_conn = {'rule': 'fixed_indegree', 'indegree': Nth}#, 'mask': {'circular': {'radius': s_sst}}}
    ...
    n2p_conn = {'rule': 'fixed_indegree', 'indegree': Nth}#, 'mask': {'circular': {'radius': s_sst}}}
    ...
    n2s_conne = {'rule': 'fixed_indegree', 'indegree': Nth}#, 'mask': {'circular': {'radius': s_sst}}}
    ...
    n2s_conni = {'rule': 'fixed_indegree', 'indegree': Nth}#, 'mask': {'circular': {'radius': s_sst}}}
    ...
    ## Remove inhibition in selected region
    if del_range>0:
        center_pos = np.array([0.5, 0.5])
        p2e = nest.GetConnections(pv_neurons, exc_neurons)
        n_p2e = len(p2e)
        print(n_p2e)
        for ci in range(n_p2e):
            target_idx = p2e[ci].target
            dist_from_center = nest.Distance(center_pos, exc_neurons[target_idx-1])[0]
            if dist_from_center < del_range:
                p2e[ci].weight = 0
        s2e = nest.GetConnections(sst_neurons, exc_neurons)
        n_s2e = len(s2e)
        print(n_s2e)
        for ci in range(n_s2e):
            target_idx = s2e[ci].target
            dist_from_center = nest.Distance(center_pos, exc_neurons[target_idx-1])[0]
            if dist_from_center < del_range:
                s2e[ci].weight = 0
    ...
    print("SST:", np.median(sst_rates), np.mean(sst_rates))
