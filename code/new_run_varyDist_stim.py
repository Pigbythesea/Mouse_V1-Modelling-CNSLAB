...
def gamma_kernel_nest(x, A, alpha, beta):
    # NEST Parameter-safe version: uses ** and nest.math.exp
    return A * (x ** (alpha - 1.0)) * nest.math.exp(-beta * x)

def get_conn_prob(dist_mm, circadian):
    """wrapped distance (mm) → connection probability"""
    d_um = dist_mm * 1000.0
    A, alpha, beta = map(float, (pars0 if circadian == 'ZT0' else pars12))
    p_raw = gamma_kernel_nest(d_um, A, alpha, beta)
    # hard-zero beyond 250 µm using NEST’s conditional (works on Parameters)
    return nest.logic.conditional(dist_mm <= 0.25, p_raw, 0.0)


def run_simulation(sim_parameters, condition, laserPosition, circadian, rngseed = '', verbose=False):
    ...
    def reset_sim():
        ...
        for popu in [exc_neurons, pv_neurons]:
    ...
    #N_pe       = int(epsilon*n_inh*cprob_pe)
    #N_pe_far   = int(N_pe*p_far_p)
    #N_pe_close = N_pe - N_pe_far
    
    N_pp       = int(epsilon*n_inh*cprob_pp)
    ...
    '''
    # Distance-dependent Pyr↔PV probabilities
    prob_matrix = get_conn_prob(nest.spatial.distance, circadian)

    # E → PV
    e2p_conn = {'rule': 'pairwise_bernoulli', 'p': prob_matrix}
    e2p_syn  = {'synapse_model':'static_synapse',
                'weight': g_syn_ep,
                'delay' : 0.1 + nest.spatial.distance/velocity}
    nest.Connect(exc_neurons, pv_neurons, e2p_conn, syn_spec=e2p_syn)

    # PV → E
    p2e_conn = {'rule': 'pairwise_bernoulli', 'p': prob_matrix}
    p2e_syn  = {'synapse_model':'static_synapse', 'receptor_type': 2,
                'weight': g_syn_pe,
                'delay' : 0.1 + nest.spatial.distance/velocity}
    nest.Connect(pv_neurons, exc_neurons, p2e_conn, syn_spec=p2e_syn)
    ...
    elif stim_type in ['PV']:
        print(stim_type)
        sim_name = f'PV_dist{laserPosition:.3f}_cond{circadian}'
    ...
    if not sim_spontaneous:
        ...
        for ni in range(n_pv):
            ...
            dist = nest.Distance(np.array([laserPosition,laserPosition]), targets[ni])
    ...
    pv_indices = np.arange(8001,10001)
    
    '''to speed the processing speed up, this original part is commented out
    exc_spikes = []
    exc_rates = []
    for count, ni in enumerate(exc_indices, 1):
        if count % 1000 == 0:
            print(f"processed {count}/8000 exc cells", flush=True)
    ...
    pv_spikes = []
    pv_rates = []
    for count, ni in enumerate(pv_indices, 1):
        if count % 200 == 0:
                print(f"processed {count}/2000 pv cells", flush=True)
    ...
    '''
    # ---------- fast, vectorised spike extraction ----------------
    # helper function: split spike recorder by sender IDs
    def split_spikes(sr, first_id, last_id):
        ...
        counts  = np.bincount(s - first_id, minlength=last_id - first_id + 1)
        starts  = np.concatenate(([0], np.cumsum(counts)))

    # list of NumPy arrays, one per neuron
        return [t[starts[i]:starts[i+1]] for i in range(last_id - first_id + 1)]

    # split excitatory (IDs 1–8000) and PV (8001–10000)
    exc_spikes = split_spikes(exc_sr, 1, 8000)
    pv_spikes  = split_spikes(pv_sr, 8001, 10000)

    # firing rates (Hz)
    sim_dur_s  = simtime / 1000.0
    exc_rates  = [len(arr) / sim_dur_s for arr in exc_spikes]
    pv_rates   = [len(arr) / sim_dur_s for arr in pv_spikes]
    # --------------------------------------------------------------

    
    
    all_spikes = exc_spikes + pv_spikes
    with open('%s/%s_spikes.pickle'%(result_dir,sim_name),'wb') as f:
        pickle.dump(all_spikes,f)

    all_positions = np.concatenate([np.array(nest.GetPosition(exc_neurons)), np.array(nest.GetPosition(pv_neurons))])
...
if __name__=="__main__":
    ...
    contrast = 0.05
    stim_type = 'PV'  # 'Spont', 'PV', 'SST'
    conditions =  [stim_type, contrast]
    #condition = conditions[cond_i]
    
    try:
        rngseed = int(sys.argv[1])
        laserPosition = float(sys.argv[2])
        circadian = sys.argv[3]
    except IndexError:
        print("Provide arguments: RNG seed, position of laser (0-0.5), and circadian phase (ZT0 or ZT12)")
        sys.exit()
    ...
    run_simulation(sim_parameters, condition, laserPosition, circadian, rngseed = rngseed)
