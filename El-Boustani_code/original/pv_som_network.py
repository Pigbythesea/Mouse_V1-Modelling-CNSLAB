...
class MyDistanceDependentProbabilityConnector(pyNN.connectors.MapConnector):    
    ...
    def connect(self, projection):
        ...
        connection_map  = numpy.zeros(projection.shape, dtype=numpy.bool)        
...
gext_baseline = 2.                  # Baseline noise
gext_rate     = 2000.               # Hz max Rate of the Gaussian source
...
nb_repeats    = 60                  # number of repeated stimulation
...
def set_gaussian(population, m_time=m_time, s_time=s_time, m_xy=m_xy, s_xy=s_xyies, rate=intensities):
    ...
    for cell in population.local_cells:            
        ...
        spikes  = []             
        rate = intensities
        for repeat in xrange(nb_repeats):
...
Projection(source_chr2, targets, AllToAllConnector(), syn)
...
ext_syn_4  = StaticSynapse(weight=g_exc, delay=0.1) 
