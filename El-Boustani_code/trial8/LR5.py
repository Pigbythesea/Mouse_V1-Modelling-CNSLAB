...
### Trial 9: contrast curves looked wonky in trial 8 so new network and lower p_far
...
p_far = 0.15 # proportion of SOM neurons with long-range connections
...
exc_syn_weak  = StaticSynapse(weight=g_exc*0.25, delay=delays)
...
inh_conn_pe = MyDistanceDependentProbabilityConnector(si_lat, allow_self_connections=False, rng=rng, n_connections=Ni_closePV)
inh_conn_pp = MyDistanceDependentProbabilityConnector(si_lat, allow_self_connections=False, rng=rng, n_connections=Ni_closePV)
...
inh_conn_ss = MyDistanceDependentProbabilityConnector(si_lat,  allow_self_connections=False, rng=rng, n_connections=int(Ni_close)*0.2)
...
ext_syn_e_2  = StaticSynapse(weight=g_tha*(1.0+(contrast-1.5)*), delay=0.1)
...
ext_syn_1  = StaticSynapse(weight=3.5*g_exc, delay=0.1) # originally 10*g_exc
ext_syn_2  = StaticSynapse(weight=2*g_exc, delay=0.1) # originally 8*g_exc
ext_syn_3  = StaticSynapse(weight=5*g_exc, delay=0.1)
ext_syn_4  = StaticSynapse(weight=8*g_exc, delay=0.1) # originally 4*g_exc
...
connections['exc2som'] = Projection(exc_cells, som_cells, exc_conn_es, exc_syn_weak, receptor_type='excitatory', space=space)
...
connections['pv2pv_far']   = Projection(pv_cells, pv_cells,   inh_conn_pp_far, inh_syn, receptor_type='inhibitory', space=space) # trial 9
...
filedir = 'v4_%s_%s_%s_%s_%s'%(chr2_origin, contrast, Chr2, ChR2_str, p_far)
