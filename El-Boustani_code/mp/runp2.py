...
def pexec(args):
    ...
    a_e = 2.0  # exponent of excitatory projection probability density
    a_p = 2.0  # exponent of PV projection probability density
    a_s = 1.0  # exponent of SOM projection probability density
    ...
    spontaneous_epochs = 4          # number of simtimes for spontaneous activity
    
    ### Chr2 stimulation ### 
    #Chr2       = 'SOM' # Stimulated population with ChR2-like input. This can be either 'PV' or 'SOM'
    Chr2_times = numpy.array([range(0,10,10)+epo+simtime*spontaneous_epochs for epo in numpy.arange(500,1000000,1000)]).flatten() # Induce an instantaneous synaptic conductance in the target population at time 0 every other trial
    ...
    def set_gaussian(population, m_time=m_time, s_time=s_time, m_xy=m_xy, s_xy=s_xyies, rate=intensities):
        ...
        for cell in population.local_cells:            
            ...
            for repeat in range(nb_repeats):
                padding = (spontaneous_epochs+repeat)*(simtime+time_spacing)
                if x>-1:
    ...
    norm_ee = get_norm(positions, a_e) * Ne
    print('Exc Projections')
    exc_conn_ee = DistanceDependentProbabilityConnector(f'{norm_ee}*d**(-{a_e}) if d>0.005 else 1',allow_self_connections=False, rng=rng)
    exc_conn_ep = DistanceDependentProbabilityConnector(f'{norm_ee}*d**(-{a_e}) if d>0.005 else 1',allow_self_connections=False, rng=rng)

    norm_es = get_norm(positions, a_e) * Ne*4
    ...
    norm_pe = get_norm(positions,a_p)* Ni 
    inh_conn_pe = DistanceDependentProbabilityConnector(f'{norm_pe}*d**(-{a_p}) if d>0.005 else 1',allow_self_connections=False, rng=rng)
    inh_conn_pp = DistanceDependentProbabilityConnector(f'{norm_pe}*d**(-{a_p}) if d>0.005 else 1',allow_self_connections=False, rng=rng)
    ...
    norm_se = get_norm(positions,a_s)*Ni
    inh_conn_se = DistanceDependentProbabilityConnector(f'{norm_se}*d**(-{a_s}) if d>0.005 else 1',allow_self_connections=False, rng=rng)
    inh_conn_sp = DistanceDependentProbabilityConnector(f'{norm_se}*d**(-{a_s}) if d>0.005 else 1',allow_self_connections=False, rng=rng)
    ...
    filedir = 'results_%s_%.3f_%s_%.3f_%s'%(chr2_origin, contrast, Chr2, ChR2_str, p_far)
    ...
    simtime = (nb_repeats+spontaneous_epochs) * simtime + (nb_repeats - 1) * time_spacing
    run(simtime)
...
if __name__ == '__main__':
    ...
    print(len(paramlist), "Simulatinos")
