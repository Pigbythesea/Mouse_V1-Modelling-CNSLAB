...
np = num_processes()
if node_id == 0:
    nest.SetStatus([0], {'print_time' : False})
def nprint(string): 
    if node_id == 0:
        print(string)
...
inh_syn_con     = StaticSynapse(weight=g_inh*contrast*12.0, delay=delays)
...
connections['som2exc'] = Projection(som_cells, exc_cells, inh_conn_se, inh_syn_stronger, receptor_type='inhibitory', space=space)
connections['som2pv']  = Projection(som_cells, pv_cells,  inh_conn_sp, inh_syn_stronger, receptor_type='inhibitory', space=space)
...
filedir = 'v22b_%s_%s_%s_%s_%s'%(chr2_origin, contrast, Chr2, ChR2_str, p_far)
