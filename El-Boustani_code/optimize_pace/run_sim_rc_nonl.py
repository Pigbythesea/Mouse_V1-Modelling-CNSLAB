...
sig              = sim_parameters['sig']
c_half         = sim_parameters['c_half']
A              = sim_parameters['A']
def nonlinearity(cont):
    c = cont
    return 1 + A/(1+numpy.exp(-(c-c_half)/sig))
...
if sim_spontaneous:
    syn_se  = StaticSynapse(weight=g_syn_se, delay=delays)
    syn_se_far  = StaticSynapse(weight=g_syn_se, delay=delays)
else:
    syn_se  = StaticSynapse(weight=g_syn_se*nonlinearity(contrast), delay=delays)
    syn_se_far  = StaticSynapse(weight=g_syn_se*nonlinearity(contrast), delay=delays)
...
if sim_spontaneous:
    result = getSpontRateMedians(all_spike_arrays, simtime)
else:
    PC_rates = getStimRateMeansPC(all_spike_arrays, exc_cells.positions, simtime)
    PV_rates = getStimRateMeansPV(all_spike_arrays, pv_cells.positions, simtime)
    SOM_rates = getStimRateMeansSOM(all_spike_arrays, som_cells.positions, simtime)
