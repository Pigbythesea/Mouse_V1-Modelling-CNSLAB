...
def gradient_descent(lr):
    ...
    results = run_batch(result_dir, simnum, paramlist, submit_lim = submit_limit, pace_queue = pace_queue, run_name = run_name)
    simnum = newsimnum
...
def compute_gradient(sim_parameters, current_rms, simnum, epoch):
    ...
    results = run_batch(result_dir, simnum, grad_paramlist,  submit_lim = submit_limit, print_results = False, pace_queue = pace_queue, run_name= run_name) 
    ...
    for pi in range(len(fitting_parameters)):
        ...
        print_grad_log("%s    %s    %s\n"%(pi, fitting_parameters[pi], grad[pi]))
    print_grad_log('\n')
...
parameter_rules['p_far_e'] = [0.1, 0.9, 0.02]
parameter_rules['p_far_p'] = [0.1, 0.9, 0.02]
parameter_rules['p_far_s'] = [0.1, 0.9, 0.02]
