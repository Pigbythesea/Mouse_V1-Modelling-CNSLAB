...
def pexec(args):
    ...
    # nonlinear contrast function parameters
    s              = sim_parameters['s']
    c_half         = sim_parameters['c_half']
    A              = sim_parameters['A']
    def nonlinearity(cont):
        c = cont
        return 1 + A/(1+numpy.exp(-s*(c-c_half)))
    ...
    # g_exc         = 1.5e-3              # nS Quantal Excitatory conductance # fitting parameter
...
#     gext_baseline =                   # Baseline noise (2)
    if not sim_spontaneous:
        gext_rate     = par_gext_rate0 + par_gext_rate1*contrast  #contrast varies between 0.02 and 1 # 2000
    ...
    if sim_parameters['chr2_tar'] != None:
    ### Chr2 stimulation ### 
        Chr2 = sim_parameters['chr2_tar']
        #Chr2       = 'PV' # Stimulated population with ChR2-like input. This can be either 'PV' or 'SOM'
        ...
        ChR2_str   = 0.2  # Strength of the ChR2 stimulation [Use 0.08 for SOM or PV for instance]
    ...
    if not sim_spontaneous:
        def set_gaussian(population, m_time=m_time, s_time=s_time, m_xy=m_xy, s_xy=s_xyies, rate=intensities):
            ...
            times     = numpy.arange(0, simtime, 1.)        
        ...
        if sim_parameters['chr2_tar'] != None:
            ...
            if Chr2 == 'PV':
                targets = pv_cells[:int(n_inh*Chr2_proba/2)]
            elif Chr2 == 'SOM':
                targets = som_cells[:int(n_inh*Chr2_proba/2)]
                                  
            # Chr2 stimulation of PV or SOM can be either distal or local
    ...
    # contrast dependent inhibitory synapse
    inh_syn_c  = StaticSynapse(weight=g_inh*nonlinearity(contrast), delay=delays)
    ...
    ext_syn_1  = StaticSynapse(weight=par_ext_syn_1*g_exc, delay=0.1) 
    ext_syn_2  = StaticSynapse(weight=par_ext_syn_2*g_exc, delay=0.1) 
    ext_syn_3  = StaticSynapse(weight=par_ext_syn_3*g_exc, delay=0.1) 
    ext_syn_4  = StaticSynapse(weight=par_ext_syn_4*g_exc, delay=0.1) 
    ...
    connections['som2exc'] = Projection(som_cells, exc_cells, inh_conn_se, inh_syn_c, receptor_type='inhibitory', space=space)
...
#         pass
    ...
    nprint("Running simulation for %g ms." %simtime)
