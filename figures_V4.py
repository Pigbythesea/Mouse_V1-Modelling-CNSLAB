...
RESULTS_DIR  = os.path.join(PROJECT_ROOT, 'work_dir', 'base_a0.07_pf0.8', 'results_6')
...
# ==== CRF (contrast sensitivity) config ====
# The width used when you generated the contrast sweeps:
FIXED_WIDTH_MM = 0.20   # change if you used a different width in testv3.py (5th CLI arg)
CONTRASTS      = np.array([0.02, 0.05, 0.10, 0.18, 0.33])  # matches simulator defaults

def resolve_paths_by_contrast(base_dir, circadian, contrast, width_mm=FIXED_WIDTH_MM):
    """
    Resolve spike/position pickles by *contrast* at a fixed rectangle width.
    Tries rectangle-tag pattern first, then legacy Gaussian-tag as fallback.
    """
    tag = f"{width_mm:.3f}"
    ...
    raise FileNotFoundError(
        f"[contrast sweep] Missing files for both patterns:\n"
        f"  {os.path.join(base_dir, base_w)}_[spikes|positions].pickle\n"
        f"  {os.path.join(base_dir, base_sig)}_[spikes|positions].pickle"
    )

def load_data_contrast(circadian, contrast, width_mm=FIXED_WIDTH_MM):
    sp_path, xy_path = resolve_paths_by_contrast(RESULTS_DIR, circadian, contrast, width_mm)
...
#%%
# ==== Heatmap config & helpers (100 µm grid) ====
GRID_UM   = 100
...
# %%
#========================================================
# CONTRAST-RESPONSE (CRF) at fixed width (E and optional PV)
#========================================================
CRF_means = {'ZT0': [], 'ZT12': []}
CRF_sems  = {'ZT0': [], 'ZT12': []}
CRF_means_PV = {'ZT0': [], 'ZT12': []}  # optional
CRF_sems_PV  = {'ZT0': [], 'ZT12': []}

for circ in CIRCADIANS:
    for c in CONTRASTS:
        sp, xy = load_data_contrast(circ, c, FIXED_WIDTH_MM)

        # E (Pyr) ROI mean/sem (uses your helper)
        mE, _, sE, _ = getStimRateMeans_v2(sp[:N_E], xy[:N_E], binlen=binlen)
        CRF_means[circ].append(mE); CRF_sems[circ].append(sE)

        # PV (optional curve)
        mP, _, sP, _ = getStimRateMeans_v2(sp[N_E:N_E+N_PV], xy[N_E:N_E+N_PV], binlen=binlen)
        CRF_means_PV[circ].append(mP); CRF_sems_PV[circ].append(sP)

CRF_means = {k: np.array(v) for k, v in CRF_means.items()}
CRF_sems  = {k: np.array(v) for k, v in CRF_sems.items()}
CRF_means_PV = {k: np.array(v) for k, v in CRF_means_PV.items()}
CRF_sems_PV  = {k: np.array(v) for k, v in CRF_sems_PV.items()}

# ---- Plot E CRFs with Naka–Rushton fits ----
plt.figure(figsize=(4.8,4.2), dpi=150)
plt.errorbar(CONTRASTS, CRF_means['ZT0'],  yerr=CRF_sems['ZT0'],  fmt='o', ms=5,
             mec='black', mfc='white', ecolor=ZT0color,  capsize=3, label='E ZT0')
plt.errorbar(CONTRASTS, CRF_means['ZT12'], yerr=CRF_sems['ZT12'], fmt='o', ms=5,
             mec=(0.4,0.4,0.4), mfc='white', ecolor=ZT12color, capsize=3, label='E ZT12')

# Fit standard Naka–Rushton (m + Rmax * c^n / (c^n + k^n))
p_zt0,  _ = fit_naka_rushton(CONTRASTS, CRF_means['ZT0'],  sigma=CRF_sems['ZT0'])
p_zt12, _ = fit_naka_rushton(CONTRASTS, CRF_means['ZT12'], sigma=CRF_sems['ZT12'])
cgrid = np.linspace(CONTRASTS.min()*0.8, CONTRASTS.max()*1.2, 400)
fit0  = naka_rushton(cgrid,  *p_zt0)
fit12 = naka_rushton(cgrid,  *p_zt12)

plt.plot(cgrid, fit0,  color=ZT0color,  linewidth=2.0, label='E ZT0 NR fit')
plt.plot(cgrid, fit12, color=ZT12color, linewidth=2.0, linestyle='--', label='E ZT12 NR fit')

# Report C50 (k) and Rmax (from params)
def extract_C50_Rmax(p):
    # expected params order: m, Rmax, k, n  (adjust if your helper differs)
    m, Rmax, k, n = p
    return k, Rmax
k0, R0     = extract_C50_Rmax(p_zt0)
k12, R12   = extract_C50_Rmax(p_zt12)
print(f"[E CRF] ZT0: C50={k0:.3f}, Rmax≈{R0:.2f};  ZT12: C50={k12:.3f}, Rmax≈{R12:.2f}")

plt.xlabel('Contrast'); plt.ylabel('E response (spks/s)')
plt.grid(True, alpha=0.25); plt.legend(frameon=False); plt.tight_layout()
# plt.savefig(os.path.join(PROJECT_ROOT, f'CRF_E_w{int(FIXED_WIDTH_MM*1000)}um.png'), bbox_inches='tight')
plt.show()

# ---- Optional: PV CRFs ----
plt.figure(figsize=(4.8,4.2), dpi=150)
plt.errorbar(CONTRASTS, CRF_means_PV['ZT0'],  yerr=CRF_sems_PV['ZT0'],  fmt='o', ms=5,
             mec='black', mfc='white', ecolor=ZT0color,  capsize=3, label='PV ZT0')
plt.errorbar(CONTRASTS, CRF_means_PV['ZT12'], yerr=CRF_sems_PV['ZT12'], fmt='o', ms=5,
             mec=(0.4,0.4,0.4), mfc='white', ecolor=ZT12color, capsize=3, label='PV ZT12')

p0_pv,  _ = fit_naka_rushton(CONTRASTS, CRF_means_PV['ZT0'],  sigma=CRF_sems_PV['ZT0'])
p12_pv, _ = fit_naka_rushton(CONTRASTS, CRF_means_PV['ZT12'], sigma=CRF_sems_PV['ZT12'])
fit0pv  = naka_rushton(cgrid, *p0_pv)
fit12pv = naka_rushton(cgrid, *p12_pv)
plt.plot(cgrid, fit0pv,  color=ZT0color,  linewidth=2.0, label='PV ZT0 NR fit')
plt.plot(cgrid, fit12pv, color=ZT12color, linewidth=2.0, linestyle='--', label='PV ZT12 NR fit')

k0pv, R0pv   = extract_C50_Rmax(p0_pv)
k12pv, R12pv = extract_C50_Rmax(p12_pv)
print(f"[PV CRF] ZT0: C50={k0pv:.3f}, Rmax≈{R0pv:.2f};  ZT12: C50={k12pv:.3f}, Rmax≈{R12pv:.2f}")

plt.xlabel('Contrast'); plt.ylabel('PV response (spks/s)')
plt.grid(True, alpha=0.25); plt.legend(frameon=False); plt.tight_layout()
# plt.savefig(os.path.join(PROJECT_ROOT, f'CRF_PV_w{int(FIXED_WIDTH_MM*1000)}um.png'), bbox_inches='tight')
plt.show()

#%% ------------- Heatmaps across CONTRASTS (fixed width) -------------
#========================================================
# Heatmap grid across CONTRASTS (fixed width)
#========================================================
def compute_maps_for_contrast(circadian, contrast, width_mm=FIXED_WIDTH_MM):
    sp, xy  = load_data_contrast(circadian, contrast, width_mm)
    starts  = control_window_starts(sp)
    rates_E  = rates_over_windows(sp, slice(0, N_E),                 starts)
    rates_PV = rates_over_windows(sp, slice(N_E, N_E + N_PV),        starts)
    mapE  = grid_average(xy, slice(0, N_E),              rates_E)
...
def sanity_check_contrasts(contrasts=CONTRASTS, width_mm=FIXED_WIDTH_MM):
    missing = []
    for c in contrasts:
        for circ in CIRCADIANS:
            try:
                resolve_paths_by_contrast(RESULTS_DIR, circ, c, width_mm)
            except FileNotFoundError as e:
                missing.append(str(e))
    if missing:
        print("\n[Missing results for some (ZT, contrast)]:")
        for m in missing: print(" -", m)
        raise FileNotFoundError("One or more required spike/position files were not found for the CRF run.")

def plot_heatmaps_grid_all_contrasts(pop='E', contrasts=CONTRASTS, width_mm=FIXED_WIDTH_MM, save=False, dpi=300):
    """
    Grid of size len(contrasts) x 3 for a single population at fixed width:
      columns: ZT0 | ZT12 | (ZT12 − ZT0)
      rows:    increasing contrast (e.g., 0.02 … 0.33)
    """
    assert pop in ('E','PV')
    sanity_check_contrasts(contrasts, width_mm)

    # Collect
    maps = {}
    diffs = {}
    for c in contrasts:
        m0  = compute_maps_for_contrast('ZT0',  c, width_mm)[pop]
        m12 = compute_maps_for_contrast('ZT12', c, width_mm)[pop]
        maps[(c, 'ZT0')]  = m0
        maps[(c, 'ZT12')] = m12
        diffs[c]          = m12 - m0

    # Global scales
    abs_vmin = np.nanmin([maps[(c,'ZT0')]  for c in contrasts] + [maps[(c,'ZT12')] for c in contrasts])
    abs_vmax = np.nanmax([maps[(c,'ZT0')]  for c in contrasts] + [maps[(c,'ZT12')] for c in contrasts])
    diff_max = np.nanmax([np.nanmax(np.abs(diffs[c])) for c in contrasts])

    # Plot
    nrows, ncols = len(contrasts), 3
    fig, axes = plt.subplots(nrows, ncols, figsize=(12, 2.4*nrows), dpi=150, constrained_layout=True)
    axes = np.atleast_2d(axes)
    cmap_abs = 'magma' if pop == 'E' else 'viridis'

    for i, c in enumerate(contrasts):
        im0 = axes[i,0].imshow(maps[(c,'ZT0')],  origin='lower', extent=[0,1000,0,1000],
                               vmin=abs_vmin, vmax=abs_vmax, cmap=cmap_abs, aspect='equal')
        im1 = axes[i,1].imshow(maps[(c,'ZT12')], origin='lower', extent=[0,1000,0,1000],
                               vmin=abs_vmin, vmax=abs_vmax, cmap=cmap_abs, aspect='equal')
        im2 = axes[i,2].imshow(diffs[c],         origin='lower', extent=[0,1000,0,1000],
                               vmin=-diff_max,   vmax= diff_max,  cmap='coolwarm', aspect='equal')
        axes[i,0].set_ylabel(f'contrast = {c:.2f}')
        for j in range(ncols):
            a = axes[i,j]
            a.set_xticks([0,250,500,750,1000])
            a.set_yticks([0,250,500,750,1000])
    ...
    axes[0,0].set_title(f'ZT0 @ w={int(width_mm*1000)}µm')
    axes[0,1].set_title(f'ZT12 @ w={int(width_mm*1000)}µm')
    axes[0,2].set_title('ZT12 − ZT0')
    ...
    if save:
        out = os.path.join(PROJECT_ROOT, f'heatmaps_grid_{pop}_CRF_{len(contrasts)}x3_w{int(width_mm*1000)}um.png')
        fig.savefig(out, dpi=dpi)
        print(f"[saved] {out}")

    return fig

# ---- Run contrast heatmaps for E and PV (comment/uncomment as needed) ----
_ = plot_heatmaps_grid_all_contrasts(pop='E',  contrasts=CONTRASTS, width_mm=FIXED_WIDTH_MM, save=False)
_ = plot_heatmaps_grid_all_contrasts(pop='PV', contrasts=CONTRASTS, width_mm=FIXED_WIDTH_MM, save=False)

#%% ------------- Active fraction vs contrast (fixed width) -------------
#========================================================
# Active fraction (> THRESH_HZ) vs contrast (fixed width)
#========================================================
THRESH_HZ = 5.0
def active_fraction_contrast(circadian, contrast, idx_slice, width_mm=FIXED_WIDTH_MM):
    sp, xy = load_data_contrast(circadian, contrast, width_mm)
    starts = control_window_starts(sp)
    rates  = rates_over_windows(sp, idx_slice, starts)
    return np.mean(rates > THRESH_HZ)

fr_crc = {'ZT0_E':[], 'ZT12_E':[], 'ZT0_PV':[], 'ZT12_PV':[]}
for c in CONTRASTS:
    fr_crc['ZT0_E'].append(  active_fraction_contrast('ZT0',  c, slice(0, N_E)))
    fr_crc['ZT12_E'].append( active_fraction_contrast('ZT12', c, slice(0, N_E)))
    fr_crc['ZT0_PV'].append( active_fraction_contrast('ZT0',  c, slice(N_E, N_E+N_PV)))
    fr_crc['ZT12_PV'].append(active_fraction_contrast('ZT12', c, slice(N_E, N_E+N_PV)))

plt.figure(figsize=(5.2,3.6), dpi=150)
plt.plot(CONTRASTS, fr_crc['ZT0_E'],   '-o', label='E ZT0',  color=ZT0color)
plt.plot(CONTRASTS, fr_crc['ZT12_E'],  '-o', label='E ZT12', color=ZT12color)
plt.plot(CONTRASTS, fr_crc['ZT0_PV'],  '--o', label='PV ZT0',  color=ZT0color)
plt.plot(CONTRASTS, fr_crc['ZT12_PV'], '--o', label='PV ZT12', color=ZT12color)
plt.xlabel('Contrast'); plt.ylabel(f'Active fraction (> {THRESH_HZ} Hz)')
plt.grid(True, alpha=0.25); plt.legend(frameon=False); plt.tight_layout()
# plt.savefig(os.path.join(PROJECT_ROOT, f'active_fraction_CRF_w{int(FIXED_WIDTH_MM*1000)}um.png'), dpi=300)
