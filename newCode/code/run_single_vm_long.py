...
def run_simulation(sim_parameters, condition, rngseed = ''):
    ...
    #mm = nest.Create('multimeter',params={"record_from":["V_m", "g_ex", "g_in"], "to_file": True, "label": "test"})
    #print(dir(mm))

    #exc_multimeter = nest.Create('multimeter', n_exc, params={'interval': 1., "record_from":["V_m", "g_ex", "g_in"], "label": "exc"})
    #pv_multimeter = nest.Create('multimeter', n_pv, params={'interval': 1., "record_from":["V_m", "g_ex", "g_in"], "label": "pv"})
    #som_multimeter = nest.Create('multimeter', n_som, params={'interval': 1., "record_from":["V_m", "g_ex", "g_in"], "label": "som"})

    #nest.Connect(exc_multimeter, exc_neurons)
    #nest.Connect(pv_multimeter, pv_neurons)
    #nest.Connect(som_multimeter, som_neurons)

    exc_recorders = []
    rec_range = 0.2
    if rec_distal:
        print("RECORDING DISTAL!")
    else:
        print("RECORDING LOCAL!")
    for ni in range(n_exc):
        if rec_distal:
            dist = nest.Distance(np.array([0.0,0.0]), exc_neurons[ni])
        else:
            dist = nest.Distance(np.array([0.5,0.5]), exc_neurons[ni])
        if dist[0] < rec_range:
            #mm = nest.Create('multimeter',1)
            mm = nest.Create("multimeter", params={"interval": 1., "record_from": ["V_m", "g_ex", "g_in"]})# , "withgid": True, "to_file": True, "label": "my_multimeter"})
            #nest.SetStatus(mm, {'record_from':['V_m']})
            #print(dist[0], 'CONNECT!', nest.GetPosition(targets[ni]))
            nest.Connect(mm, exc_neurons[ni])
            exc_recorders.append(mm)
    ...
    if not sim_spontaneous:
        ...
        Chr2_times = np.array([sim_delay + 500.+1000.*i for i in range(nb_repeats)]) # Induce an instantaneous synaptic conductance in the target population at time 0 every other trial
        ...
        ##source_chr2_in = nest.Create('spike_generator', params={'spike_times': Chr2_times})
        ...
        for stime in Chr2_times:
            ...
            chr2_rates.append(0)
            chr2_switch_times.append(stime+40)
        ...
        chr_syn_in = {'synapse_model':'static_synapse', 'weight': chr2_str*10.}
        for ni in range(n_pv):
            ...
            if dist[0] < stim_range:
                ...
                nest.Connect(source_chr2_in, targets[ni], syn_spec = chr_syn_in)
            elif stim_type == 'SOM':
                roll = rng.random()
    ...
    all_data = []
    for mm in exc_recorders:
        all_data.append(mm.get()['events'])
    #for ni in range(n_exc):
    #    all_data.append(exc_multimeter.get()['events'])
    #for ni in range(n_pv):
    #    all_data.append(pv_multimeter.get()['events'])
    #for ni in range(n_som):
    #    all_data.append(som_multimeter.get()['events'])

    if rec_distal:
        with open('%s/%s_rec_distal.pickle'%(result_dir,sim_name), 'wb') as f:
            pickle.dump(all_data, f)
    else:
        with open('%s/%s_rec.pickle'%(result_dir,sim_name), 'wb') as f:
            pickle.dump(all_data, f)
