    ...
    if not sim_spontaneous:
        ...
        for ni in range(n_pv):
            ...
            if dist[0] < stim_range:
                ...
                laser_intensity = chr2_rate * np.exp(-(dist[0]**2) / (2*laser_spread**2))
                chr2_rates_mult = [ laser_intensity * rate for rate in chr2_rates]

                nest.SetStatus(source_chr2, {'rate_times':chr2_switch_times, 'rate_values':chr2_rates_mult})
                nest.Connect(source_chr2, targets[ni], syn_spec = chr_syn)
                chr2_sources.append(source_chr2)
